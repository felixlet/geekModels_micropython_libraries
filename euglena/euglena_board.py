...
def Euglena_get_board_config_js():
    try:
...
def Euglena_save_board_config_js(config_dict):
    config_str = json.dumps(config_dict)
    config_file = open("\config\euglena.config",'wr')
    config_file.write(config_str)
    config_file.close()

def Euglena_config_sta_ssid(o_ssid):
    config_dict = Euglena_get_board_config_js()
    config_dict['sta_ssid'] = o_ssid
    Euglena_save_board_config_js(config_dict)

def Euglena_config_sta_pwd(o_pwd,auth_type):
...
def Euglena_get_sta_ip():
    wlan = WLAN()
    return wlan.ifconfig()[0]

def Euglena_get_version_info():
    build = os.uname()[-2][-10:]
    return 'build:' + build

def Euglena_mount_microSD():
    try:
        microSD = SD()
    except OSError:
        return False
